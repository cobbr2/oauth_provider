  ...
  module Backends
    class Sqlite3 < Abstract
      ...
      def create_user_request(token)
        @db.execute("INSERT INTO request_tokens (shared_key, secret_key, callback, authorized, consumer_shared_key) " \
      ...
      def find_user_request(shared_key)
        @db.execute("SELECT shared_key, secret_key, callback, authorized, consumer_shared_key FROM request_tokens WHERE shared_key = '#{shared_key}' LIMIT 1") do |row|
